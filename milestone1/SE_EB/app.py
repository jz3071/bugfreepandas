...
def register():
    inputs = log_and_extract_input(register)
    reg_data = inputs["body"]
    rsp_status = None
    rsp_txt = None

    try:
        user_service = _get_user_service()
        logger.error("/api/register: _user_service = " + str(user_service))

        rsp = user_service.create_user(reg_data)
        if rsp is not None:
            reg_data = rsp
            rsp_status = 200
            rsp_txt = "OK"
        ...
        if reg_data is not None:
            full_rsp = Response(json.dumps(reg_data), status=rsp_status, content_type="application/json")
        else:
            full_rsp = Response(rsp_txt, status=rsp_status, content_type="text/plain")

    except Exception as e:
        log_msg = "/api/register: Exception = " + str(e)
        logger.error(log_msg)
        rsp_status = 500
        rsp_txt = "INTERNAL SERVER ERROR. Please take COMSE6156 -- Cloud Native Applications."
        full_rsp = Response(rsp_txt, status=rsp_status, content_type="text/plain")

    log_response("/api/register", rsp_status, reg_data, rsp_txt)
...
def user_email(email):
    ...
    try:
        ...
        if inputs["method"] == "GET":
            ...
            rsp = user_service.update_user(user_info=update_data, email=email)
        ...
        log_msg = "/delete_email: Exception = " + str(e)
        logger.error(log_msg)
        rsp_status = 500
        rsp_txt = "INTERNAL SERVER ERROR. Please take COMSE6156 -- Cloud Native Applications."
        full_rsp = Response(rsp_txt, status=rsp_status, content_type="text/plain")

    log_response("/delete_email", rsp_status, rsp_data, rsp_txt)
